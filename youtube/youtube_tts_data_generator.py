...
class YTSpeechDataGenerator(object):
    ...
    def concat_audios(self, max_limit=11):
        """
        ...
        if os.path.exists(self.split_audios_csv) and os.path.isfile(
            ...
            alltext = ""
            while ix < df.shape[0]:
                current_audio = df.iloc[ix][0]
                text = df.iloc[ix][1]
                current_length = float(df.iloc[ix][2])
                ...
                while ix < df.shape[0] and current_length + float(df.iloc[ix][2]) <= max_limit:
                    
                # while current_length is less than or equal to max_limit
                    next_audio = df.iloc[ix][0]
                ...
                print("final text is" + str(text))
                print("final length is " + str(current_length))

                text = text.strip()
                # add text to alltext with a newline between
                alltext += current_audio + "|" + text + "\n"
                # create a new variable that is current_audio without the .wav suffix
                new_audio = current_audio.replace(".wav", "")
                new_name = f"{new_audio}"
                ...
                with open(
                    ...
                    f.write(text)
                print ("Exporting " + new_name + ".wav")
            # remove any empty newlines from alltext
            alltext = alltext.strip()
            with open(
                os.path.join(self.concat_dir, "metadata" + ".csv"), "w"
            ) as f:
                f.write(alltext)
    ...
    def prepare_dataset(
        ...
        self.split_audios()
        self.concat_audios(max_concat_limit)
